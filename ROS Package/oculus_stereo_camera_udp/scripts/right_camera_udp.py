...
class robotSocketServer:
    def __init__(self):
        # Subscribe to right camera topic
        self.right_image_sub = rospy.Subscriber("/right/camera/image_raw/compressed", CompressedImage, self.right_img_cb)
    ...
    def right_img_cb(self, ros_data):
	    eye = "Right"
        self.send_img(eye, ros_data)

def main(args):
    rospy.init_node('right_camera_udp')
